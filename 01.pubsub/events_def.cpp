...
LoginEvent::LoginEvent(char* buffer) : iEvent(buffer)
{
...
bool LoginEvent::decode()
{
    ...
    if (!reader.parse(content_, attributes))
    {
    ...
    if ((attributes["password"] != Json::nullValue) && attributes["password"].isString())
    {
